  ...
  <q-page padding>
    <q-toolbar inset>
      <q-breadcrumbs active-color="positive" style="font-size: 15px">
        <q-breadcrumbs-el icon="inventory_2" to="stock" label="Inventario" />
        <q-breadcrumbs-el v-if="!row.name" icon="add" label="Crear" class="text-green" />
        <q-breadcrumbs-el :label="row.name" to="stockDetail" class="text-green" />
      </q-breadcrumbs>
    </q-toolbar>
          ...
          <div class="col-auto">
            <q-btn color="grey-7" round flat icon="more_vert">
              <q-menu cover auto-close>
                <q-list>
                  <q-item clickable @click="deleteMaterial()">
        ...
        <q-form class="q-gutter-md">
          <q-card-section horizontal>
            <q-card-section class="col-6">
              <q-input :readonly="!edit" class="q-ma-sm" filled v-model="row.name" label="Nombre" dense />
              <q-input :readonly="!edit" class="q-ma-sm" filled v-model="row.description" label="Descripción" dense />
            </q-card-section>
            <q-separator vertical />
            <q-card-section class="col-6">
              <q-input :readonly="!edit" class="q-ma-sm" type="number" filled v-model="row.quantity" label="Cantidad"
                dense />
              <q-input :readonly="!edit" class="q-ma-sm" filled v-model="row.uom" label="UOM" dense />
            </q-card-section>
          </q-card-section>
          <q-separator />
          <q-card-section>
  ...
  methods: {
    ...
    validateForm() {
      if (!this.row.name ||
        ...
        if (!sessionStorage.getItem("data")) this.postMaterial()
        else this.putMaterial()
      }
    },
    getData() {
    ...
    putMaterial() {
      ...
      formData.append("quantity", this.row.quantity)
      formData.append("uom", this.row.uom)
      if (!this.validateObjects()) {
        api.put(`materials/${data.id}/`, formData)
          .then((res) => {
            if (res.status == 200) {
              this.edit = false
